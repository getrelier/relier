...
async def status() -> None:
    ...
    raw_worker_ids = await redis.smembers("rl:workers")  # type: ignore[misc]
    ...
    for w_id_bytes in sorted(raw_worker_ids):
        ...
        if not is_alive:
            ...
            await redis.srem("rl:workers", w_id_bytes)  # type: ignore[misc]
            continue
    ...
    for w_id in active_workers:
        ...
        inflight_count = await redis.zcard(f"rl:inflight:{w_id}")
...
async def reset_metrics(
    ...
    await redis.delete(f"rl:worker:{worker_id}:completed")
    await redis.delete(f"rl:worker:{worker_id}:failed")
